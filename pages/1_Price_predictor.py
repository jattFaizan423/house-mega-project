...
@st.cache_resource
def load_files():
    # Go up from pages/ to repo root
    BASE_DIR = os.path.dirname(os.path.dirname(__file__))  
    data_path = os.path.join(BASE_DIR, 'df.pkl')            # rename your df file to df.pkl
    pipeline_path = os.path.join(BASE_DIR, 'pipeline.joblib')  # converted pipeline file

    # Check if files exist
    if not os.path.exists(data_path) or not os.path.exists(pipeline_path):
        st.error("❌ Required files not found! Make sure 'df.pkl' and 'pipeline.joblib' are in the root folder.")
        st.stop()

    # Load data
    df = pd.read_pickle(data_path)

    # Load pipeline safely with joblib
    try:
