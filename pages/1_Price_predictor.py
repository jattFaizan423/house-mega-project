...
def load_files():
    ...
    data_path = os.path.join(BASE_DIR, 'df.pkl')        # Updated to .pkl data file
    pipeline_path = os.path.join(BASE_DIR, 'pipeline.joblib')  # Updated to joblib

    # Check if files exist
    if not os.path.exists(data_path) or not os.path.exists(pipeline_path):
        st.error("❌ Required files not found! Make sure 'df.pkl' and 'pipeline.joblib' are in the same folder.")
        st.stop()
